    ...
    MobileCalendar.prototype = {
        ...
        bindEvent: function(type) {
            ...
            function setTimeGearTooth() {
                ...
                var date_mm_top = Math.round(parseFloat(date_mm.getAttribute("top")));
                var date_dd_top = Math.round(parseFloat(date_dd.getAttribute("top")));
                var minHH;
                var minMM; 
                ...
                if(date_mm_top=="8" && date_dd_top=='8'){
                    minHH = new Date().getHours();
                    minMM = new Date().getMinutes();
                    if(hhVal<minHH){
                        hhVal = minHH;
                        // 改val
                ...
                if (time_hh && time_hh.getAttribute("val")) {
                    ...
                    for (var g = minHH; g <= 23; g++) {
                        i += "<div class='tooth'>" + g + "</div>";
                    }
                    time_hh.innerHTML = i;

                   
                    time_hh.style["-webkit-transform"] = 'translate3d(0,' + (8 - (hhVal-minHH) * 2) + 'em,0)';
                    time_hh.setAttribute('top', 8 - (hhVal-minHH) * 2 + 'em');
                ...
                if (time_mm && time_mm.getAttribute("val")) {
                    ...
                    for (var g = minMM; g <= 59; g++) {
                        i += "<div class='tooth'>" + g + "</div>";
                    }
                    time_mm.innerHTML = i;

                    time_mm.style["-webkit-transform"] = 'translate3d(0,' + (8 - (mmVal-minMM) * 2) + 'em,0)';
                    time_mm.setAttribute('top', 8 - (mmVal-minMM) * 2 + 'em');
                } else {
                    return
                }
            }
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    switch (target.dataset.datetype) {
                        ...
                        case "date_dd":
                            ...
                            var date_mm_top = Math.round(date_mm.getAttribute("top"));
                            var date_dd_top = Math.round(date_dd.getAttribute("top"));
                                         
                            if(date_mm_top=="8" && date_dd_top=='8'){                             
                                time_hh.setAttribute("val", new Date().getHours());
                                time_mm.setAttribute("val", new Date().getMinutes());                  
                            }
                        ...
                        case "time_hh":
                            ...
                            if (stopGear) {
                                ...
                                var gearVal = Math.abs(pos - 8) / 2 +minHH;
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
                            break;
                        case "time_mm":
                            ...
                            if (stopGear) {
                                ...
                                var gearVal = Math.abs(pos - 8) / 2 +minMM;
                                setGear(target, gearVal);
                                clearInterval(target["int_" + target.id]);
                            }
                            break;
